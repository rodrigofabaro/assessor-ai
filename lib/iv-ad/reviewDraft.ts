...
const reviewDraftModelSchema = z.object({
  ...
  feedbackReviewReport: z.string().trim().min(1).max(12000),
...
function buildPrompt(input: IvAdReviewDraftRequest) {
  ...
  return [
    ...
    "6) Provide a structured tutor-feedback review report for margin notes and end feedback.",
    "7) Provide short evidence snippets with source labels.",
    "",
    "feedbackReviewReport requirements:",
    "- Write a concise but specific audit-safe review of the tutor's in-text notes and end feedback.",
    "- Use VASCR logic where helpful: valid, authentic, sufficient, current, reliable.",
    "- Do not re-grade the work and do not invent evidence.",
    "- Distinguish presentation issues, technical weaknesses, evidence gaps, and criterion gaps.",
    "- Keep the report holistic across the whole submission, not fragmented page-only commentary.",
    "- Return feedbackReviewReport as plain text using exactly these headings:",
    "1. General weaknesses in the current notes",
    ...
    "- In section 4, include a markdown table with columns: Original note | Problem with note | Improved version.",
    "- In section 5, organise the replacement final feedback around strengths, gaps, next steps, and an overall holistic summary.",
    "",
    "Metadata:",
  ...
  const response = await fetchOpenAiJson(
    ...
    apiKey,
    {
      ...
      body: JSON.stringify({
        ...
        max_output_tokens: Math.max(1600, Math.min(5000, Number(process.env.OPENAI_IV_AD_MAX_OUTPUT_TOKENS || 3600))),
        text: {
          format: {
            type: "json_schema",
            ...
            schema: {
              ...
              properties: {
                ...
                feedbackReviewReport: { type: "string" },
