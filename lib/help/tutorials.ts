...
const TUTORIALS_BY_SLUG: Record<string, Omit<HelpTutorial, "slug" | "title" | "route">> = {
  ...
  "submission-detail": {
    ...
    purpose: "Review one submission end-to-end with an operator-first flow: fix only what is wrong, confirm outputs, and move on.",
    howItWorks: [
      "Header shows a single status line (state + current step + next action).",
      "Left workflow rail guides the sequence from cover review to approval & outputs.",
      "Review-ready submissions auto-open Approval & outputs for faster confirmation.",
      "Technical diagnostics are hidden under an optional accordion in Approval & outputs.",
    ],
    whyItMatters: [
      "Reduces cognitive load during batch review sessions.",
      "Keeps the assessor focused on the next required action, not system internals.",
      "Single-record truth source for disputes and QA checks.",
    ],
    preflight: [
      "Confirm the current status line and highlighted workflow step.",
      "If reviewing a saved submission, start in Approval & outputs and verify the selected assessment run.",
    ],
    steps: [
      {
        id: "validate-context",
        ...
        how: [
          "Follow the left workflow rail in order (Cover -> Assignment -> Student) when the header says Action needed.",
      ...
      {
        id: "preview-and-approval",
        title: "Review preview and approval outputs",
        what: "Confirm grade, feedback, and marked output readiness before saving.",
        how: [
          "Use the Final confirmation block in Approval & outputs.",
          "If needed, inspect Criterion Decisions and feedback editor before saving.",
          "Use Save to audit or Save to audit & next when preview is fresh.",
        ],
        why: "The fastest safe path is confirm-first, then save and move on.",
      },
      {
        id: "manual-adjustments",
        title: "Apply manual adjustments only when needed",
        what: "Override criterion decisions or feedback text when assessor judgement differs.",
        how: [
    ...
    issues: [
      {
        issue: "Page looks complete but save is disabled",
        cause: "Preview is stale after a context change (cover/assignment/student/settings).",
        fix: "Regenerate preview, then return to Approval & outputs and save.",
      },
      {
        issue: "Need to inspect confidence/readiness internals",
        cause: "Technical diagnostics are hidden by default in the operator layout.",
        fix: "Open Technical details (optional) inside Approval & outputs.",
      },
    ],
    screenshots: [
      {
