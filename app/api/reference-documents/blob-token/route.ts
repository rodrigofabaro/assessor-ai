...
export async function POST(req: Request) {
  try {
    ...
    if (fileSize > MAX_REFERENCE_UPLOAD_BYTES) {
      return NextResponse.json(
    ...
    const storagePath = toStorageRelativePath("reference_uploads", storedFilename);
    const validUntil = Date.now() + BLOB_TOKEN_TTL_MS;

    const clientToken = await generateClientTokenFromReadWriteToken({
      token: rawToken,
      pathname: storagePath,
      maximumSizeInBytes: MAX_REFERENCE_UPLOAD_BYTES,
    ...
    return NextResponse.json({
      ...
      storagePath,
      storedFilename,
      maxBytes: MAX_REFERENCE_UPLOAD_BYTES,
      validUntil,
    });
  } catch (error) {
    ...
    return NextResponse.json(
      {
        ...
        code: "REFERENCE_BLOB_TOKEN_FAILED",
      },
      { status: 500 },
    );
  }
