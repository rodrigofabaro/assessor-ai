  ...
  const taskRows = useMemo(() => {
    ...
    return extracted.map((task: any, idx: number) => {
      const override =
        ...
        null;
      if (!override)
        return { task, extractedTask: task, overrideApplied: false };
      const merged = { ...task };
    ...
    <section className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm min-w-0">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-700">
            This is the brief&apos;s “question paper”. The grader will later
            check student evidence against these task blocks.
          </p>
        </div>

        <div className="flex items-center gap-2">
          <Pill cls={tasksOverride ? tone("info") : tone("muted")}>
            {tasksOverride ? "OVERRIDE" : "EXTRACTED"}
          </Pill>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
        <div className="text-xs text-zinc-500">
          Default view is compact. Expand only the task you are reviewing.
        </div>
        <div className="flex items-center gap-2">
          <button
            type="button"
      ...
      <div className="mt-3 grid gap-3">
        {taskRows && taskRows.length ? (
          ...
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
            ...
            <div className="mt-1">
              Run Extract on the BRIEF PDF in the inbox. If the template is odd,
              use the override editor below.
            </div>
